...
Definition empty_state LOP : state := {|
...
Lemma get_all_blks_tree_empty LOP :
  get_all_blks_tree (empty_oram LOP) = [].
Proof.
  induction LOP.
  - auto.
  - simpl.
...
Lemma empty_state_wf LOP : well_formed LOP (empty_state LOP).
Proof.
  split.
  - constructor.
  - unfold state_oram.
...
Fixpoint get_traces_aux (acc : list path) (LOP : nat) (ops : list (block_id * operation)) : Poram (list path) :=
  ...
  | (id,op) :: ops' =>
    p <- access LOP id op;;
    get_traces_aux (fst p :: acc) LOP ops'
  end.

Definition get_traces (LOP : nat) (ops : list (block_id * operation)) (init : state) : dist (list path) :=
